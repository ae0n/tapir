...
object ShadowedEndpointChecker {
  ...
  private def checkIfShadows(e1: Endpoint[_, _, _, _], e2: Endpoint[_, _, _, _]): Boolean = {
    ...
    val commonSegments = e1Segments
      .zip(e2Segments)
      .filter(p =>
        p._1.equals(WildcardPathSegment) || p._1.equals(p._2) || p._1.equals(PathVariableSegment)
      )
  ...
  def extractSegments(endpoint: Endpoint[_,_,_,_]): Vector[PathComponent] = {
    println("extract segments")
      endpoint.input.traverseInputs({
        ...
        case b: EndpointInput.Basic[_] => Vector(FixedPathSegment("xxx"))
        case _ => Vector.empty
      })
  }
}
